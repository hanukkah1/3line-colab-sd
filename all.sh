...
function run {
    ...
    cd $BASEPATH && python launch.py --share --api --disable-safe-unpickle --enable-insecure-extension-access --no-download-sd-model --no-half-vae --no-half --opt-sdp-attention --disable-console-progressbars --theme dark $BASEPATH/models/CLIP
}
